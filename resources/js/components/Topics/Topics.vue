...
<script>
    ...
    export default {
        name: "Topics",
        components: {TopicsForm, TopicsPagination, TopicsRow, TopicsTbody, TopicsThead, TopicsTable},
        props: {
            course: {
                type: String,
                required: true
        ...
        methods: {
            async fetchTopics() {
                const { data: { data: topics, meta: pagination } } = await window.axios
                    .get(`/courses/${this.course}/topics/json?page=${this.page}`);
                this.topics = topics;
